...
const SystemShowCase = ({showCase}: SystemShowCaseProps) => {
    ...
    return (
        ...
        <div className={'w-full h-full flex flex-col gap-4 justify-center'}>
            ...
            <div className={`flex justify-center items-center ${bebas.className}`}>
                {
                    systemDetail && splitName(systemDetail.label).map((e, index) => (
                        <div className='text-3xl tablet:text-3xl s-desktop:text-3xl m-desktop:text-5xl l-desktop:text-6xl text-neutral-800'
            ...
            <div className='w-full max-h-60 shrink-0 flex flex-col gap-4 py-2 overflow-auto '>

                <p ref={detailRef} className={`text-xs tablet:text-xs s-desktop:text-xs m-desktop:text-base l-desktop:text-base text-neutral-800 flex  ${notoSansThai.className} `}>
                    {systemDetail?.detail}
                </p>
            ...
            <div
                ...
                className={`${robotoBold.className} rounded text-sm tablet:text-sm s-desktop:text-sm m-desktop:text-base l-desktop:text-base w-32 h-8 flex justify-center items-center text-neutral-900 gap-2`}>
            ...
            <div className='w-full min-h-40 max-h-60 shrink-0 grid grid-cols-2 gap-4 content-start overflow-y-auto p-2'>
                {systemDetail.functions.map((func: {
                    ...
                    <div
                        ...
                        key={`func-${index}`}
                         className={'h-8 flex justify-center gap-2 items-center bg-neutral-900 drop-shadow rounded shadow shadow-neutral-800 overflow-hidden'}>
                        <div
                            className={'h-full w-full flex justify-center items-center text-gray-300'}>

                            <p className={`text-xs tablet:text-xs s-desktop:text-xs m-desktop:text-sm l-desktop:text-sm whitespace-nowrap  ${notoSansThai.className}`}>{func.label}</p>
                        </div>
